...
# ======================================================
class MultiLabelTrainerProtocol2:
    ...
    def train(self, optim_name='adam', num_epochs=10, learning_rate=1e-3,
        ...
        for epoch in range(num_epochs):
            ...
            with torch.no_grad():
                for X, y, _, users in self.val_loader:
                    X, y = X.to(self.device), y.to(self.device)
                    logits = self.net(X)
                    loss = custom_multilabel_loss(logits, y)
            ...
            for user_name in unique_users:
                ...
                mask = (val_users == user_name)
                user_index = np.argmax(val_labels[mask][0])

                y_true = val_labels[mask][:, user_index]
                y_score = val_scores[mask][:, user_index]

                eer, auc, thr = calculate_eer(y_true, y_score)
