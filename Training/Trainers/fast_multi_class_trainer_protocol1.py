...
class MultiLabelTrainerCNN:
    ...
    def __init__(self,
        ...
        dataset_labels = torch.from_numpy(train_loader.dataset.labels).float()

        total_count = dataset_labels.shape[0]

        self.F1_all = (dataset_labels.sum(dim=0) / total_count).clamp(min=1e-6).to(self.device)
        self.F2_all = ((1.0 - dataset_labels).sum(dim=0) / total_count).clamp(min=1e-6).to(self.device)
    ...
    def train(self,
        ...
        def custom_multilabel_loss(logits, labels):
            ...
            w1 = self.C_pos / self.F1_all
            w2 = self.C_neg / self.F2_all
