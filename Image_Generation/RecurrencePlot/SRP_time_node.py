...
# ============================================================
# SRP + NODE Δt (Strip Encoding)
# ============================================================

def compute_srp_time_node(seq, percentile=95):
    ...
    dt_node[:-1] = ts[1:] - ts[:-1]
    dt_node[-1] = dt_node[-2] if T > 1 else 0
    ...
    # --------------------------------------------------------
    # 竖纹 strip encoding（关键）
    # --------------------------------------------------------

    dt_strip = np.tile(dt_norm[None, :], (T, 1))
...
def draw_srp_time_node(seq, save_path, percentile, chunk_size):

    img = compute_srp_time_node(seq, percentile)
        ...
        for file in sorted(os.listdir(user_dir)):
            ...
            for chunk_size in sizes:
                ...
                for i in range(n_chunks):
                    ...
                    draw_srp_time_node(seq, save_path, percentile, chunk_size)
...
def main():
    ...
    print("\nSRP time_node generation finished.")
