...
def compute_srp_velocity_diff(seq, percentile=95):
    ...
    # pairwise velocity difference
    dv = np.abs(v_norm[:, None] - v_norm[None, :]).astype(np.float32)

    # normalize dv
    dv_max = np.percentile(dv, 95)
    dv = np.clip(dv, 0, dv_max)
...
def draw_srp_velocity_diff(seq, save_path, percentile, chunk_size):

    img = compute_srp_velocity_diff(seq, percentile)
        ...
        for file in sorted(os.listdir(user_dir)):
            ...
            for chunk_size in sizes:
                ...
                for i in range(n_chunks):
                    ...
                    draw_srp_velocity_diff(seq, save_path, percentile, chunk_size)
...
def main():
    ...
    print("\nSRP Velocity Diff generation finished.")
