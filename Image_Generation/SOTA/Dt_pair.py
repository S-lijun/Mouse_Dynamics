...
def draw_srp_difference(prev_seq, curr_seq, save_path, epsilon):

    rp_prev = compute_srp_pair(prev_seq, epsilon)
    rp_curr = compute_srp_pair(curr_seq, epsilon)
    ...
    img = ((D - d_min) / denom * 255).astype(np.uint8)
