...
TENSOR_SUBDIR = "Chong_per_user_vxvy"
GLOBAL_VX_CDF = None
GLOBAL_VY_CDF = None
...
def render_sequence_vxvy(seq, norm_width, norm_height):
    """
    Per-user screen coords; white bg; polyline colored by signed-CDF vx/vy.
...
def draw_sequence_vxvy(seq, save_path, norm_width, norm_height):
    img_rgb = render_sequence_vxvy(seq, norm_width, norm_height)
    if img_rgb is None:
        return
...
def process_dataset_tensors(dataset, data_root, out_dir, user_max_xy):
    ...
    print("Rendering: white bg, stroke R=0 G=vx B=vy |", TARGET_SIZE, "x", TARGET_SIZE)
    print("\n[Phase] Generating per-user XYPlot vx/vy-colored tensors...")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Per-user XYPlot: white bg, stroke R=0 G=vx B=vy.",
    )
    ...
    print("\nPer-user XYPlot vx/vy-colored generation finished.")
