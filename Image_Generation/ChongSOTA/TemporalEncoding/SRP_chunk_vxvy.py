...
def render_srp_chunk_vxvy(seq, epsilon, output_size=0):
...
def draw_srp_chunk_vxvy(seq, save_path, epsilon, output_size=0):
    img_rgb = render_srp_chunk_vxvy(seq, epsilon, output_size)
    if img_rgb is None:
        return
...
def process_dataset_tensors(dataset, data_root, out_dir, sizes, epsilon, output_size=0):
    ...
    print("\n[Phase] Generating pair-wise SRP + vx/vy stripe tensors (Images_convert format)...")
