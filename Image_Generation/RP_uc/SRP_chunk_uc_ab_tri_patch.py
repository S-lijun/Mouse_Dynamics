# -*- coding: utf-8 -*-
"""
Triangle-split SRP + screen-patch pair encoding (no gamma):

  Upper triangle (incl. diagonal): pair-wise local SRP distance
  Lower triangle: each event pair (i, j) with i > j
    1) Per-user screen = [min_x, max_x] x [min_y, max_y]
       (supports negative coords e.g. DFL)
    2) Tile into n_y x n_x patches by span:
         n_y = short_side (default 10)
         n_x = round(((max_x-min_x)/(max_y-min_y)) * n_y)
       K = n_y * n_x
    3) Map each event to a patch id via (coord-min)/span (row-major)
    4) Unordered patch combo (incl. same-cell): n_comb = C(K,2)+K
         index = hi*(hi+1)//2 + lo   # lo<=hi, 0-based
    5) brightness = index / (n_comb - 1)  in [0,1]

Usage:
  python SRP_chunk_uc_ab_tri_patch.py --dataset balabit \\
    --data_root Data/Balabit-dataset/training_files \\
    --out_dir Images/Balabit/SRP_uc_ab_tri_patch --sizes 125
"""
...
def process_dataset_tensors(
    ...
    print("Mode: upper=SRP, lower=patch-pair combo (short_side={})".format(short_side))
    print("Per-user bounds loaded for", len(bounds.get("users", {})), "users.")
    print("\n[Phase] Generating triangle SRP+patch tensors...")
...
def process_dataset(
    ...
    print("Mode: upper=SRP, lower=patch-pair combo (short_side={})".format(short_side))
    print("Per-user bounds loaded for", len(bounds.get("users", {})), "users.")
    print("\n[Phase] Generating triangle SRP+patch PNGs...")
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Upper=SRP; lower=screen patch-pair combination mapped to [0,1]."
        ),
    )
    ...
    print("Mode: triangle split (upper=SRP, lower=patch-pair combo)")
