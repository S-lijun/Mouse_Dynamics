# -*- coding: utf-8 -*-
"""
R=B=SRP (same as ChongSOTA/SRP_chunk), G=position stripes:

  G lower triangle (i > j): G[i,j] = (x[j]-min_x)/(max_x-min_x)
  G upper triangle (i < j): G[i,j] = (y[j]-min_y)/(max_y-min_y)
  G diagonal: 0.5 * (x_norm[i] + y_norm[i])

  min/max: per-user from training bounds JSON (supports negative coords e.g. DFL).

Usage:
  python SRP_chunk_uc_rb_g_xy.py --dataset balabit \\
    --data_root Data/Balabit-dataset/training_files \\
    --out_dir Images/Balabit/SRP_uc_rb_g_xy --sizes 125
"""
...
def process_dataset_tensors(
    ...
    print("Mode: R=B=SRP, G=xy min-max stripes ((x-min)/(max-min))")
    print("Per-user bounds loaded for", len(bounds.get("users", {})), "users.")
    print("\n[Phase] Generating SRP RGB (R=B=SRP, G=xy) tensors...")
    ...
    for chunk_size in sizes:
        ...
        for user in users:
            ...
            for file in list_session_csvs(user_dir):
                ...
                for seq in windows:
                    img = render_srp_rb_g_xy(seq, epsilon, output_size, user_bounds)
                    if img is None:
                        continue
                    if img.shape[:2] != (H, W):
...
def process_dataset(
    ...
    print("Mode: R=B=SRP, G=xy min-max stripes ((x-min)/(max-min))")
    print("Per-user bounds loaded for", len(bounds.get("users", {})), "users.")
    print("\n[Phase] Generating SRP RGB (R=B=SRP, G=xy) PNGs...")
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "R=B=SRP; G=vertical stripes "
            "(x-min_x)/(max_x-min_x) lower, (y-min_y)/(max_y-min_y) upper."
        )
    )
    ...
    print("Mode: R=B=SRP, G=xy vertical stripes")
    ...
    for u in sorted(bounds["users"].keys(), key=natural_key):
        ...
        print(
            "  ", u, "->",
            "min=({}, {}) max=({}, {})".format(
                ub["min_x"], ub["min_y"], ub["max_x"], ub["max_y"],
            ),
        )
