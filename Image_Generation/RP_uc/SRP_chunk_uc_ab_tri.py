# -*- coding: utf-8 -*-
"""
Triangle-split SRP + position (no gamma):

  Upper triangle (incl. diagonal): pair-wise local SRP distance
  Lower triangle: vertical stripes by event order (left -> right)
    loc_k = d_BR(k) - d_TR(k)          # each d in [0,1] => loc in [-1,1]
    stripe_k = (loc_k + 1) / 2         # linear map [-1,1] -> [0,1]
    for i > j: out[i, j] = stripe[j]   # column j = event j

  BR=(max_x, max_y), TR=(max_x, min_y)
  Per-user min/max from training bounds JSON.

Usage:
  python SRP_chunk_uc_ab_tri.py --dataset balabit \\
    --data_root Data/Balabit-dataset/training_files \\
    --out_dir Images/Balabit/SRP_uc_ab_tri --sizes 125
"""
...
def process_dataset_tensors(
    ...
    print("Mode: upper=SRP, lower=vertical (BR-TR) stripes (no gamma)")
    print("Per-user bounds loaded for", len(bounds.get("users", {})), "users.")
    print("\n[Phase] Generating triangle SRP+position tensors...")
    ...
    for chunk_size in sizes:
        ...
        for user in users:
            ...
            for file in list_session_csvs(user_dir):
                ...
                for seq in windows:
                    img = render_srp_tri_ab(seq, epsilon, output_size, user_bounds)
                    if img is None:
                        continue
                    if img.shape[:2] != (H, W):
...
def process_dataset(
    ...
    print("Mode: upper=SRP, lower=vertical (BR-TR) stripes (no gamma)")
    print("Per-user bounds loaded for", len(bounds.get("users", {})), "users.")
    print("\n[Phase] Generating triangle SRP+position PNGs...")
...
def main():
    parser = argparse.ArgumentParser(
        description="Upper=SRP; lower=vertical stripes of (d_BR-d_TR) mapped to [0,1]."
    )
    ...
    print("Mode: triangle split (upper=SRP, lower=vertical BR-TR stripes)")
