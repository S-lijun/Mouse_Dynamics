# -*- coding: utf-8 -*-
"""
R=B=full SRP; G=vertical stripes by per-event screen patch id (no pair combo):

  Same patch grid as other patch scripts:
    ...
    K = n_y * n_x
    patch id in [0, K) (row-major)
    stripe[j] = patch_id(j) / (K - 1)  in [0,1]
    G[i, j] = stripe[j]   # vertical stripe: column j = event j

Usage:
  python SRP_chunk_uc_rb_g_patch_stripe.py --dataset balabit \\
    --data_root Data/Balabit-dataset/training_files \\
    --out_dir Images/Balabit/SRP_uc_rb_g_patch_stripe --sizes 125
"""
...
def process_dataset_tensors(
    ...
    print("Mode: R=B=SRP, G=patch-id vertical stripes (short_side={})".format(short_side))
    print("Per-user bounds loaded for", len(bounds.get("users", {})), "users.")
    print("\n[Phase] Generating SRP RGB (R=B=SRP, G=patch stripe) tensors...")
    ...
    for chunk_size in sizes:
        ...
        for user in users:
            ...
            for file in list_session_csvs(user_dir):
                ...
                for seq in windows:
                    img = render_srp_rb_g_patch_stripe(
...
def process_dataset(
    ...
    print("Mode: R=B=SRP, G=patch-id vertical stripes (short_side={})".format(short_side))
    print("Per-user bounds loaded for", len(bounds.get("users", {})), "users.")
    print("\n[Phase] Generating SRP RGB (R=B=SRP, G=patch stripe) PNGs...")
...
def main():
    parser = argparse.ArgumentParser(
        description="R=B=full SRP; G=vertical stripes of per-event patch index.",
    )
    ...
    print("Mode: R=B=SRP, G=patch-id vertical stripes")
