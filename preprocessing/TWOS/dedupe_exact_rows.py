"""
TWOS preprocess step 1: drop exact duplicate (timestamp, x, y) rows.

Operates in-place on Data/TWOS/User*/session*.csv (keep first).
Then regenerates training_files / testing_files_protocol1 via 2/3-1/3 split.
"""
import os
import re
import csv
...
def main():
    ...
    print(f"Deduping exact (timestamp,x,y) under: {ROOT}")
    ...
    print(f"removed exact dups: {total_before - total_after:,} "
