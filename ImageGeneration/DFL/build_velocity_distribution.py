...
def build_raw_velocity_distribution(training_root):
    ...
    for u_idx, user in enumerate(users, 1):
        ...
        # =====================================================
        # DFL: scan all CSV files (no session_ prefix)
        # =====================================================
        ...
        for s_idx, file in enumerate(session_files, 1):
            ...
            # Create node-level velocity array
            v = np.zeros(len(xs))
            v[1:] = v_segment

            v = v[np.isfinite(v)]

            print(f"      Extracted velocities (node-level): {len(v)}")
...
def main():
    ...
    parser.add_argument("--out_file", type=str, default="DFL_velocity_distribution_raw.npz")
